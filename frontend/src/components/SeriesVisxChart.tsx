...
import { Axis, AxisLeft, type TickFormatter } from '@visx/axis';
...
export function SeriesVisxChart({
    width,
    height,
    margin = { top: 6, right: 0, bottom: 40, left: 60 },
}: ChartProps) {
    ...
    const dateTickValues: Date[] = [new Date(series.data[0].date), ...series.data.map((d, i) => i % 60 === 0 ? new Date(d.date) : undefined).filter(Boolean) as Date[]];
    ...
    return (
        <svg width={width} height={height}>
            ...
            <Axis scale={dateScale} top={innerHeight + 2*margin.top } label="Time" tickValues={dateTickValues} tickFormat={dateTickFormat} />
        </svg>
    );
};